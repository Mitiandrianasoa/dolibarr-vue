<!-- src/views/front/BulkSalaryByMonthView.vue -->
<template>
  <div class="bulk-salary-view">
    <div class="page-header">
      <div>
        <h1>Génération de salaires par mois</h1>
        <p class="subtitle">Génère uniquement les jours non déjà couverts par un salaire existant</p>
      </div>
    </div>
    ...
    <div class="card generate-card">
      ...
      <div class="generate-grid">
        <div class="form-group">
          <label>Mois *</label>
          <select v-model.number="salaryForm.mois">
            <option v-for="m in 12" :key="m" :value="m">{{ m }}</option>
          </select>
        </div>
        <div class="form-group">
          <label>Année *</label>
          <input type="number" min="2000" v-model.number="salaryForm.annee" required />
        </div>
        <div class="form-group">
          <label>Salaire / jour *</label>
          <input type="number" step="0.01" min="0" v-model.number="salaryForm.salaireParJour" required />
        </div>
        <div class="form-group">
          <label>Majoration férié (%)</label>
          <input type="number" step="0.01" min="0" v-model.number="salaryForm.pourcentageFerie" />
        </div>
      </div>
      <button
        class="btn-primary"
...
const canGenerate = computed(() =>
  ...
  salaryForm.value.mois >= 1 &&
  salaryForm.value.annee > 0 &&
  salaryForm.value.salaireParJour > 0
)
...
const generateSalaries = async () => {
  ...
  if (!confirm(`Générer les salaires (${salaryForm.value.salaireParJour} €/jour) pour ${filteredEmployees.value.length} salarié(s) ?`)) {
    return
  }
  ...
  results.value = []
  try {
    results.value = await bulkService.createBulkSalaryByMonth(filteredEmployees.value, salaryForm.value)
  } finally {
