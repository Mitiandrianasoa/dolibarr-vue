...
async function insertMvt(
  ...
  // Recherche du ticket GLPI correspondant
  const allTickets = await fetchAllTickets()

  allTickets.sort((a: any, b: any) => a.id - b.id)

  const ticket = allTickets[position - 1]

  if (!ticket) {
    console.warn(
      `[ImportCout] position ${position} → ticket introuvable`
    )
  ...
  // open
  if (mvt === 'open') {
    const latest = await getLatestTicketCost(ticket.id)
    ...
    const reopenCost =
      Math.round(latest.fixedCost * (valeur / 100) * 100) / 100
...
async function importerMouvements() {
  ...
  try {
    // Importer chaque mouvement
    for (const m of movements.value) {
      try {
        const result = await insertMvt(
          String(m.position),
          m.mvt,
          m.value,
        )
        ...
        console.error(
          `[ImportCout] position ${m.position} erreur:`,
          e,
        )
      }
    }
    ...
    console.error(
      '[ImportCout] Erreur import des mouvements:',
      e,
    )
  }
